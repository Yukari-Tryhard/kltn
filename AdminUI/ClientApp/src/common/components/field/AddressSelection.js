...
function AddressSelection({ formik, ...props }) {
	...
	const [listState, setListState] = useState(props?.value?.country ? [...State.getStatesOfCountry(props?.value?.country)] : [{ isoCode: '', name: '---' }]);
	const [listCity, setListCity] = useState(props?.value?.state ? [...City.getCitiesOfState(props?.value?.country, props?.value?.state)] : [{ isoCode: '', name: '---' }]);
