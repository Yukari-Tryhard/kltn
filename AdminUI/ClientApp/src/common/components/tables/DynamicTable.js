...
function DynamicTable(props) {
	const { data, columns, handleDeleteRange, onAddEditOpen, tableRowAction, permission, hideAction, hideButtons, noPaging } = props;
		...
		(hooks) => {
			hooks?.visibleColumns?.push((columns) => [
                ...
                <IndeterminateCheckbox {...getToggleAllRowsSelectedProps()} />
					...
					Cell: ({ row }) => (
						<HStack>
							{/* {permission?.delete && <IndeterminateCheckbox {...row.getToggleRowSelectedProps()} type="checkbox" />} */}
							<Box>
								<Menu isLazy>
							...
							<HStack flex="1">
								{permission?.create && (
									<Tooltip placement="top" hasArrow label="Add new record for table">
										<Button shadow="2xl" colorScheme="blue" onClick={onAddEditOpen}>
											Add New
										</Button>
									</Tooltip>
								)}
								...
								{headerGroups?.map((headerGroup) => (
									<Tr {...headerGroup?.getHeaderGroupProps()}>
										{headerGroup?.headers?.map((column, index) => {
											...
											if (index !== 0) {
												return (
													<Th textTransform="capitalize" fontSize="lg" color="white" {...column?.getHeaderProps()} display={column?.hidden ? 'none' : 'undefine'} bg="#338BFF">
														<Flex alignItems="center" gap="5px">
															{column?.haveFilter && (
																<Menu>
																	<MenuButton>
											...
											return (
												<Th textTransform="capitalize" fontSize="lg" bg="#338BFF" {...column?.getHeaderProps()}>
